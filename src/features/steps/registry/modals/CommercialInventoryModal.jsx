...
export default function CommercialInventoryModal({ unit, unitsList = [], buildingLabel, onClose, onSave }) {
  ...
  const handleCopy = () => {
    ...
    if (!sourceRooms || sourceRooms.length === 0) {
      toast.error(`Помещение №${copySourceNum} пустое`);
      return;
    }
  ...
  const statsContent = (
    <div className="grid grid-cols-4 gap-4">
      <StatBadge
        label="Основная площадь"
        value={stats.main}
        subLabel="Торговая / Офисная"
        color="bg-emerald-50 text-emerald-700 border-emerald-100"
      />
      <StatBadge
        label="Вспомогательная"
        value={stats.aux}
        subLabel="Склады, С/У, Коридоры"
      ...
      <div className="space-y-3">
        ...
        {rooms.map((room, idx) => {
          ...
          return (
            <div
              key={room.id}
              className="grid grid-cols-12 gap-3 items-center p-2 bg-white rounded-xl border border-slate-200 shadow-sm hover:border-emerald-300 transition-colors"
            >
        ...
        {!isReadOnly && (
          <button
            onClick={addRoom}
            disabled={!commercialRoomTypes.length}
            className="w-full py-3 border-2 border-dashed border-slate-300 rounded-xl flex items-center justify-center gap-2 text-slate-500 font-bold text-xs hover:border-emerald-400 hover:text-emerald-600 hover:bg-emerald-50 transition-all"
          >
