...
export default function IntegrationBuildings() {
  ...
  const { integrationStatus, setIntegrationStatus, setBuildingCadastre } =
  ...
  const handleSimulateResponse = async () => {
    ...
    for (const b of buildings) {
      // Доступ к полю с учетом того, что оно может быть не замаплено в camelCase
      // @ts-ignore
  ...
  return (
    <div className="w-full pb-20 animate-in fade-in duration-500 space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-500 text-sm">
              Первичная постановка зданий и сооружений на кадастровый учет
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500">
                {status === SYNC_STATUS.IDLE &&
                  'Проверьте список зданий ниже и нажмите "Отправить" для регистрации.'}
                {status === SYNC_STATUS.WAITING &&
                  'Пожалуйста, ожидайте ответа от внешней системы.'}
                {status === SYNC_STATUS.COMPLETED &&
                  'Все здания зарегистрированы. Можно переходить к следующему этапу.'}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 w-full md:w-auto">
            ...
            {status !== SYNC_STATUS.IDLE && !isReadOnly && (
              <button
                onClick={handleReset}
                className="text-xs text-slate-400 hover:text-red-500 underline decoration-dashed px-2"
              >
                Сброс
              </button>
            )}
      ...
      <div className="space-y-3">
        <SectionTitle icon={Building2} className="px-1">
          Реестр зданий для передачи
        </SectionTitle>
            ...
            <tbody className="divide-y divide-slate-100">
              {buildings.length === 0 ? (
                ...
                buildings.map((item, idx) => {
                  // Доступ к полю с учетом возможных вариантов
                  // @ts-ignore
                  const cadastreNum = formatBuildingCadastre(
                    item.cadastreNumber || item.cadastre_number || ''
                  );
