      ...
      {blockBasements.length === 0 ? (
        ...
        <div className="space-y-3">
          {blockBasements.map((base, idx) => (
            <div
              key={base.id}
              className="p-3 bg-slate-800 rounded-xl border border-slate-700 relative group text-white shadow-inner"
            >
              <button
                disabled={isReadOnly}
                onClick={() => removeBasement(base.id)}
                className={`absolute top-2 right-2 opacity-0 group-hover:opacity-100 text-slate-400 hover:text-white hover:bg-slate-700 p-1 rounded transition-all ${isReadOnly ? 'hidden' : ''}`}
              >
                <X size={14} />
              </button>
              <div className="flex gap-3 items-center">
                <div className="w-10 h-10 flex items-center justify-center bg-slate-700 border border-slate-600 rounded-lg font-bold text-slate-300 shadow-sm shrink-0">
                  P-{idx + 1}
                </div>
