...
export default function MopEditor({ buildingId, onBack }) {
  ...
  const { mops, upsertMop, clearAllMops } = useDirectCommonAreas(
    currentBlock?.id,
  ...
  const autoFillMops = async () => {
    ...
    floors.forEach(f => {
      entrances.forEach(e => {
        ...
        if (isUnderground)
          defaultType = mopLabelByCode.OTHER || mopTypeOptions[0]?.label || defaultType;
        else if (f.type === 'basement')
          defaultType = mopLabelByCode.TECH || mopTypeOptions[0]?.label || defaultType;
        else if (f.type === 'roof')
          defaultType = mopLabelByCode.OTHER || mopTypeOptions[0]?.label || defaultType;
  ...
  if (!building || !currentBlock)
    return <div className="p-12 text-center text-slate-500">Загрузка...</div>;
  ...
  const renderBadge = type => {
    ...
    return (
      <span
        className={`text-[10px] px-1.5 py-0.5 rounded font-bold uppercase border ${style.color}`}
      >
        {style.label}
      </span>
    );
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            disabled={!showEditor || isReadOnly}
            className={`px-4 py-2.5 bg-red-50 text-red-600 rounded-xl text-xs font-bold flex items-center gap-2 transition-colors border border-red-100 shadow-sm ${isReadOnly ? 'opacity-50 cursor-not-allowed' : 'hover:bg-red-100'}`}
          >
            <Trash2 size={14} /> Очистить
          </button>

          <button
            onClick={autoFillMops}
            disabled={!showEditor || isReadOnly}
            className={`px-4 py-2.5 rounded-xl text-xs font-bold flex items-center gap-2 transition-colors border shadow-sm ${showEditor && !isReadOnly ? 'bg-white text-purple-600 border-purple-100 hover:bg-purple-50' : 'bg-slate-50 text-slate-300 border-slate-100 cursor-not-allowed'}`}
          >
        ...
        <>
          {!validationState.isValid && !isReadOnly && (
            <div className="p-3 bg-red-50 border border-red-100 rounded-xl flex items-center gap-2 text-xs text-red-600 animate-in slide-in-from-top-2">
              ...
              <span>
                Заполните данные для всех {validationState.missingCount} помещений. Используйте
                "Авто-генерацию".
              </span>
            </div>
          )}
            ...
            >
              <table className="w-max min-w-full border-collapse table-fixed">
                <thead className="bg-slate-50 border-b border-slate-200 text-[10px] text-slate-500 font-bold uppercase sticky top-0 z-30 shadow-md">
                  <tr>
                    <th className="p-4 w-36 min-w-[140px] sticky left-0 bg-slate-50 z-40 border-r border-slate-200 shadow-[4px_0_8px_-4px_rgba(0,0,0,0.1)]">
                      Этаж
                    </th>
                    {entrances.map(e => (
                      <th
                        key={e.id}
                        className="p-4 w-[340px] min-w-[340px] border-r border-slate-200 bg-slate-50/95 backdrop-blur"
                      >
                ...
                <tbody className="divide-y divide-slate-100 bg-white">
                  {floors.map(f => (
                    <tr
                      key={f.id}
                      className="group hover:bg-slate-50/50 focus-within:bg-blue-50/50 transition-colors duration-200"
                    >
                      ...
                      {entrances.map(e => {
                        ...
                        return (
                          <td
                            key={e.id}
                            className="p-3 w-[340px] min-w-[340px] align-top border-r relative group/cell"
                          >
                            {targetQty > 0 ? (
                              <div className="flex flex-col gap-2">
                                ...
                                {Array.from({ length: targetQty }).map((_, mIdx) => {
                                  ...
                                  return (
                                    <div
                                      key={mop.id || mIdx}
                                      className={`flex gap-1 items-center bg-white border rounded-lg p-1 shadow-sm transition-all focus-within:ring-2 focus-within:ring-blue-100 ${isValid ? 'border-slate-200' : 'border-red-300 bg-red-50/20'}`}
                                    >
                                      ...
                                      <div className="relative w-16 shrink-0">
                                        <DebouncedInput
                                          ...
                                          value={mop.area || ''}
                                          onChange={val =>
                                            updateMop(mop.id, f.id, e.id, mIdx, 'area', val)
                                          }
                                          disabled={isReadOnly}
                                        />
                                      </div>
        ...
        <div className="flex flex-col items-center justify-center h-[50vh] text-center space-y-4 animate-in fade-in">
          <div className="w-20 h-20 bg-slate-100 rounded-full flex items-center justify-center text-slate-400">
            ...
            <div className="absolute">
              <Ban size={20} className="text-slate-400 translate-x-4 translate-y-4" />
            </div>
          </div>
