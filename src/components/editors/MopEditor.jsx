...
export default function MopEditor({ buildingId, onBack }) {
    ...
    const autoFillMops = () => { 
        ...
        floorList.forEach(f => { 
            entrances.forEach(e => { 
                let types = [];
                if (isParking) { types = ['Паркинг (зона проезда)', 'Рампа']; } 
                else if (f.type === 'basement') { types = ['Техническое подполье', 'ИТП', 'Насосная']; } 
                else if (f.type === 'technical' || f.type === 'loft') { types = ['Технический этаж', 'Машинное отделение лифтов']; } 
                else if (f.type === 'roof') { types = ['Кровля']; } 
                else { types = ['Лестничная клетка', 'Лифтовой холл', 'Межквартирный коридор']; if (f.id === 'floor_1') types.push('Вестибюль', 'Тамбур'); }
    ...
    return (
        <div className="space-y-6 pb-20 w-full mx-auto animate-in fade-in duration-500">
            <div className="flex items-center justify-between border-b border-slate-200 pb-6 mb-4">
                <div className="flex gap-4 items-center">
                    ...
                    <div><h2 className="text-2xl font-bold text-slate-800">{building.label}</h2><p className="text-slate-400 text-xs font-bold uppercase">Инвентаризация МОП</p></div>
                </div>
            ...
            <div className="flex gap-2 mb-4">
                 {blocksList.map((b,i) => (<TabButton key={b.id} active={activeBlockIndex===i} onClick={()=>setActiveBlockIndex(i)}>{b.type==='Паркинг'?'Паркинг':`Блок ${i+1}`}</TabButton>))}
            </div>
            ...
            <Card className="shadow-lg border-0 ring-1 ring-slate-200 rounded-xl overflow-hidden flex flex-col w-full">
                <div className="flex-1 overflow-auto relative w-full" style={{ maxHeight: 'calc(100vh - 250px)' }}>
                    ...
                    {/* w-max заставляет таблицу выходить за пределы экрана и включать скролл */}
                    <table className="w-max border-collapse">
                        <thead className="bg-slate-50 border-b border-slate-200 text-[10px] text-slate-500 font-bold uppercase sticky top-0 z-30 shadow-sm">
                            <tr>
                                {/* ЭТАЖ: Sticky Left, ширина 140px */}
                                <th className="p-4 w-36 sticky left-0 bg-slate-50 z-40 border-r border-slate-200 shadow-[4px_0_8px_-4px_rgba(0,0,0,0.1)]">Этаж</th>
                                
                                {entrances.map(e => (
                                    /* ПОДЪЕЗДЫ: ширина 320px (w-80) */
                                    <th key={e} className="p-4 w-80 border-r border-slate-200 bg-slate-50/95 backdrop-blur">
                        ...
                        <tbody className="divide-y divide-slate-100 bg-white">
                            {floorList.map((f, floorIdx) => (
                                <tr key={f.id} className="group hover:bg-blue-50/20 focus-within:bg-blue-50 transition-colors duration-200">
                                    {/* ЭТАЖ: Sticky Left */}
                                    <td className="p-3 w-36 sticky left-0 bg-white group-focus-within:bg-blue-50 transition-colors duration-200 border-r align-top relative z-20 shadow-[4px_0_8px_-4px_rgba(0,0,0,0.05)]">
                                    ...
                                    {entrances.map(e => {
                                        ...
                                        return (
                                            <td key={e} className="p-3 w-80 align-top border-r relative group/cell">
                                                <div className="flex flex-col gap-2 mb-2">
                                                    {mops.map((m) => (
                                                        <div key={m.id} className="flex gap-1 items-center bg-white border border-slate-200 rounded-lg p-1 shadow-sm animate-in fade-in zoom-in-95 duration-200 focus-within:ring-2 focus-within:ring-blue-100 focus-within:border-blue-300">
                                                            ...
                                                            />
                                                            <button onClick={()=>removeMop(e, f.id, m.id)} className="p-1 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded transition-colors"><X size={12}/></button>
                                                        </div>
                                                    ))}
                                                    <button onClick={() => addMop(e, f.id)} className="self-start px-2 py-1.5 bg-slate-100 text-slate-500 rounded-lg text-[10px] font-bold border border-transparent hover:border-blue-200 hover:text-blue-600 hover:bg-white transition-all flex items-center gap-1"><Plus size={12}/> Добавить МОП</button>
                                                </div>
