...
import {
  ...
  Store,
  LayoutGrid,
  X, // [FIX] Добавлен X (на всякий случай)
} from 'lucide-react';
...
export default function EntranceMatrixEditor({ buildingId, onBack }) {
  ...
  const isFieldEnabled = (floor, field) => {
    const isLinkedStylobate =
      floor?.blockId && currentBlock?.id && floor.blockId !== currentBlock.id;
  ...
  if (residentialBlocks.length === 0)
    return (
      <div className="p-12 text-center text-slate-500">
        Нет жилых блоков для инвентаризации подъездов
      </div>
    );
      ...
      {showEditor ? (
              ...
              <thead className="sticky top-0 z-30 shadow-md">
                <tr className="bg-slate-100 border-b-2 border-slate-300">
                  ...
                  {entrances.map(e => (
                    <th
                      key={e.id}
                      className="p-2 w-[180px] min-w-[180px] border-r border-slate-300 bg-slate-100"
                    >
              ...
              <tbody className="divide-y divide-slate-200 bg-white">
                {floors.map((f, idx) => {
                  ...
                  return (
                    <tr
                      key={f.id}
                      className={`${rowBg} hover:bg-blue-50/30 transition-colors group h-14`}
                    >
                      ...
                      {entrances.map(e => (
                        <td
                          key={e.id}
                          className={`p-2 w-[180px] min-w-[180px] border-r border-slate-200 relative group/cell align-middle ${f.isDuplex ? 'bg-purple-50/10' : ''}`}
                        >
                          ...
                          {!isReadOnly && (
                            <div className="absolute top-1/2 -translate-y-1/2 right-0 pr-1 opacity-0 group-hover/cell:opacity-100 transition-opacity z-10 pointer-events-none">
                              <div className="flex flex-col gap-1 pointer-events-auto bg-white/95 backdrop-blur-sm shadow-md border border-slate-200 rounded-md p-0.5">
                                <button
                                  onClick={() => fillEntrancesRow(f.id, e.number)}
                                  title="Заполнить ряд вправо"
        ...
        <div className="flex flex-col items-center justify-center h-[50vh] text-center space-y-4 animate-in fade-in bg-slate-50/50 rounded-2xl border-2 border-dashed border-slate-200">
          ...
          <p className="text-slate-500 max-w-md">
            Матрица подъездов заполняется только для жилых блоков и подземных паркингов.
          </p>
        </div>
      )}
    </div>
