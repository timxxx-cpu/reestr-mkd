...
export default function ApartmentsRegistry({ onSaveUnit, projectId }) {
    ...
    const { data, stats, filterOptions } = useMemo(() => {
        ...
        const eMap = {}; entrances.forEach(e => { eMap[e.id] = e; });
        ...
        const searched = enriched.filter(item => {
            ...
            return String(item.number || '').toLowerCase().includes(low);
        });

        const filtered = searched.filter(item => {
            const byBuilding = filters.building === 'all' || item.buildingId === filters.building;
            const byEntrance = filters.entrance === 'all' || String(item.entrance) === String(filters.entrance);
        ...
        const buildingOptions = buildings
            .map(b => ({ value: b.id, label: `Дом ${b.houseNumber || '-'} (${b.label || 'Без названия'})` }))
            .sort((a, b) => a.label.localeCompare(b.label, 'ru'));

        const context = filters.building === 'all'
            ? searched
            : searched.filter(x => x.buildingId === filters.building);

        const entranceOptions = Array.from(new Set(context.map(x => x.entrance).filter(v => v !== '-' && v !== null)))
            .sort((a, b) => Number(a) - Number(b))
            .map(v => ({ value: String(v), label: `Подъезд ${v}` }));

        const floorOptions = Array.from(new Set(context.map(x => x.floorLabel).filter(v => v && v !== '-')))
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-3">
                ...
                <Select value={filters.building} onChange={(e) => setFilters((p) => ({ ...p, building: e.target.value, entrance: 'all', floor: 'all' }))}>
                    <option value="all">Все дома</option>
                    {filterOptions.buildings.map((opt) => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
                </Select>

                <Select value={filters.entrance} onChange={(e) => setFilters((p) => ({ ...p, entrance: e.target.value }))}>
                    <option value="all">Все подъезды</option>
                    {filterOptions.entrances.map((opt) => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
                </Select>

                <Select value={filters.floor} onChange={(e) => setFilters((p) => ({ ...p, floor: e.target.value }))}>
                    <option value="all">Все этажи</option>
                    {filterOptions.floors.map((opt) => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
                </Select>
            </div>
                            ...
                            {data.length > 0 ? rowVirtualizer.getVirtualItems().map((virtualRow) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <td className="p-4 text-center text-slate-700 font-medium">{item.rooms ?? 0}</td>
                                        <td className="p-4 text-right font-mono font-bold text-slate-800 bg-emerald-50/30 border-l border-emerald-100/50">{(parseFloat(item.area) || 0).toFixed(2)}</td>
                                        <td className="p-4 text-right font-mono text-slate-600 border-l border-slate-100">{(parseFloat(item.livingArea) || 0) > 0 ? (parseFloat(item.livingArea) || 0).toFixed(2) : '-'}</td>
