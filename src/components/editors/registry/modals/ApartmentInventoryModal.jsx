...
export default function ApartmentInventoryModal({ unit, unitsList = [], buildingLabel, onClose, onSave }) {
    ...
    const statsContent = (
        <div className="grid grid-cols-4 gap-4">
            <StatBadge label="Жилая площадь" value={stats.living} subLabel={`${stats.living_rooms_count} жил. комн.`} color="bg-blue-50 text-blue-700 border-blue-100"/>
            <StatBadge label="Полезная площадь" value={stats.useful} subLabel="Без балконов" color="bg-emerald-50 text-emerald-700 border-emerald-100"/>
        ...
        >
            <div className="space-y-3">
                <div className={`grid gap-3 px-2 text-[9px] font-bold text-slate-400 uppercase tracking-wider grid-cols-12`}>
                    ...
                    {isDuplex ? <><div className="col-span-2">Уровень</div><div className="col-span-6">Тип</div></> : <div className="col-span-8">Тип помещения</div>}
                ...
                {rooms.map((room, idx) => {
                    ...
                    return (
                        <div key={room.id} className="grid grid-cols-12 gap-3 items-center p-2 bg-white rounded-xl border border-slate-200 shadow-sm hover:border-blue-300 transition-colors">
                            ...
                            {isDuplex && (
                                <div className="col-span-2">
                                    <Select value={room.level || '1'} onChange={(e) => updateRoom(room.id, 'level', e.target.value)} className="text-xs py-2 bg-slate-50 border-slate-100" disabled={isReadOnly}>
