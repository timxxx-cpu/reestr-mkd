...
import { Card, Input, Button, useReadOnly } from '../../ui/UIKit';
        ...
        <Card className="p-12 flex flex-col items-center justify-center text-center space-y-6 min-h-[400px] border-2 border-dashed border-slate-200 shadow-none">
            {currentPhoto ? (
                <div className="relative group max-w-lg">
                    <img src={currentPhoto} className="rounded-2xl shadow-xl ring-4 ring-white" alt="Facade"/>
                    {!isReadOnly && (
                        <button onClick={handleClear} className="absolute top-4 right-4 bg-white text-red-500 p-2 rounded-xl shadow-lg opacity-0 group-hover:opacity-100 transition-all transform hover:scale-110">
                            <Trash2 size={20}/>
                        </button>
                    )}
                </div>
                ...
                <>
                    ...
                    <div className="flex gap-2 w-full max-w-md mt-4">
                        ...
                        <Button disabled={isReadOnly} onClick={handleSave} className="shadow-lg shadow-blue-200">Загрузить</Button>
                    </div>
                </>
            )}
        </Card>
