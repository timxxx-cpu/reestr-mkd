...
export default function ConstructiveCard({ details, updateDetail, errors }) {
    ...
    const getErrorClass = (key) => safeErrors[key] ? 'border-red-500 focus:border-red-500 bg-red-50 text-red-900 placeholder:text-red-300' : '';

    return (
        <Card className={`p-5 shadow-sm transition-all duration-300 ${hasCardErrors ? 'border-red-300 ring-2 ring-red-50 shadow-red-100' : ''}`}>
            <div className="flex justify-between items-center mb-4">
                <SectionTitle icon={Hammer} className="mb-0">Конструктив</SectionTitle>
                {hasCardErrors && (
            ...
            <div className="space-y-5">
                ...
                {/* Сейсмичность */}
                <div className={`pt-4 border-t ${safeErrors.seismicity ? 'border-red-100' : 'border-slate-100'}`}>
                    <div className="flex items-center gap-2 mb-2">
                        <Activity size={16} className={safeErrors.seismicity ? "text-red-500" : "text-rose-500"}/>
                        <Label className={`font-bold mb-0 ${safeErrors.seismicity ? "text-red-700" : "text-slate-700"}`}>
                    ...
                    {safeErrors.seismicity && <p className="text-[10px] text-red-500 mt-1 font-bold">Необходимо указать сейсмичность</p>}
                </div>
            </div>
        </Card>
    );
