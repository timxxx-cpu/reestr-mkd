...
import { Maximize, Settings2, Store, Lock, ArrowUpFromLine, Plus } from 'lucide-react';
    ...
    <Card className="p-6 shadow-sm">
      ...
      <div className="grid grid-cols-2 gap-8 mb-6">
        <div className="space-y-1">
          ...
          />
          {isFloorFromDisabled && (
            <p className="text-[9px] text-slate-400">Начальный этаж фиксирован</p>
          )}
        </div>
        <div className="space-y-1">
          <Label>По этаж</Label>
      ...
      <div className="flex flex-wrap gap-4 mt-4 mb-6">
        {[
          ...
          >
            ...
            {disabled && k === 'hasBasementFloor' && (
              <span className="text-[8px] text-red-400 ml-auto pl-1">Занят</span>
            )}
          </label>
        ))}
      </div>
        ...
        <div className="flex items-center gap-2 mb-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-amber-600/80 leading-tight">
              Выберите этаж, <b>НАД</b> которым нужно добавить тех.этаж
            </p>
          </div>
        </div>
        ...
        <div className="mt-6 p-4 bg-blue-50/50 border border-blue-100 rounded-xl animate-in fade-in slide-in-from-top-2">
          ...
          <div className="flex flex-wrap gap-1.5 mb-4">
            ...
            {details.hasBasementFloor && (
              <button
                disabled={isReadOnly}
                onClick={() => toggleFloorAttribute('commercialFloors', 'tsokol')}
                className={`px-2 h-8 rounded-md text-xs font-bold shadow-sm transition-all border relative ${details.commercialFloors?.includes('tsokol') ? 'bg-blue-600 border-blue-600 text-white transform scale-105' : 'bg-white border-blue-200 text-blue-400 hover:bg-blue-100'} ${isReadOnly ? 'opacity-50 cursor-not-allowed' : ''}`}
              >
                Цоколь
              </button>
            )}
            ...
            {details.hasAttic && (
              <button
                disabled={isReadOnly}
                onClick={() => toggleFloorAttribute('commercialFloors', 'attic')}
                className={`px-2 h-8 rounded-md text-xs font-bold shadow-sm transition-all border relative ${details.commercialFloors?.includes('attic') ? 'bg-blue-600 border-blue-600 text-white transform scale-105' : 'bg-white border-blue-200 text-blue-400 hover:bg-blue-100'} ${isReadOnly ? 'opacity-50 cursor-not-allowed' : ''}`}
              >
                Мансарда
              </button>
            )}
            {details.hasLoft && (
              <button
                disabled={isReadOnly}
                onClick={() => toggleFloorAttribute('commercialFloors', 'loft')}
                className={`px-2 h-8 rounded-md text-xs font-bold shadow-sm transition-all border relative ${details.commercialFloors?.includes('loft') ? 'bg-blue-600 border-blue-600 text-white transform scale-105' : 'bg-white border-blue-200 text-blue-400 hover:bg-blue-100'} ${isReadOnly ? 'opacity-50 cursor-not-allowed' : ''}`}
              >
                Чердак
              </button>
            )}
            {details.hasExploitableRoof && (
              <button
                disabled={isReadOnly}
                onClick={() => toggleFloorAttribute('commercialFloors', 'roof')}
                className={`px-2 h-8 rounded-md text-xs font-bold shadow-sm transition-all border relative ${details.commercialFloors?.includes('roof') ? 'bg-blue-600 border-blue-600 text-white transform scale-105' : 'bg-white border-blue-200 text-blue-400 hover:bg-blue-100'} ${isReadOnly ? 'opacity-50 cursor-not-allowed' : ''}`}
              >
                Кровля
              </button>
            )}
