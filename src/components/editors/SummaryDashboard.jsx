...
const StatCard = ({
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-2xl border bg-white p-5 shadow-sm transition-all duration-300 hover:shadow-md ${warning ? 'border-red-300 ring-2 ring-red-50' : 'border-slate-200'} group`}
    >
      <div className="flex items-start justify-between mb-4">
        <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400">{label}</h3>
        <div
          className={`p-2.5 rounded-xl ${activeStyle} transition-transform group-hover:scale-110`}
        >
          <_Icon size={20} />
        </div>
      </div>
      <div className="flex flex-col gap-3">
        ...
        {subMetric && (
          <div className="flex items-center justify-between">
            ...
            <span
              className={`text-sm font-bold px-2 py-0.5 rounded-lg border flex items-center gap-1 ${warning ? 'bg-red-50 text-red-600 border-red-100' : 'bg-slate-50 text-slate-700 border-slate-100'}`}
            >
              {subMetric.value}{' '}
              <span className="text-[10px] text-slate-400 ml-0.5">{subMetric.unit}</span>
            </span>
          </div>
        )}
      </div>
...
const SummaryDashboard = () => {
    ...
    return (
      <div className="flex flex-col items-center justify-center h-[50vh] text-center space-y-4 animate-in fade-in">
        ...
        <p className="text-slate-500 max-w-md">
          Заполните данные о зданиях, этажах и помещениях, чтобы сформировать ТЭП.
        </p>
      </div>
    );
  }
  ...
  const barData = [{ name: 'Площади', Проект: stats.totalAreaProj, Факт: stats.totalAreaFact }];
    ...
    <div className="w-full pb-20 space-y-6 animate-in fade-in duration-500 px-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          ...
          subMetric={{
            ...
            value:
              stats.totalAreaFact > 0
                ? stats.totalAreaFact.toLocaleString(undefined, { maximumFractionDigits: 0 })
                : '-',
            unit: 'м²',
          }}
        />
        ...
        <StatCard
          ...
          mainMetric={{
            label: 'Площадь',
            value: stats.infrastructure.area.toLocaleString(undefined, {
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <Card className="xl:col-span-2 p-6 shadow-sm flex flex-col min-h-[400px]">
          <div className="flex justify-between items-center mb-6">
            <SectionTitle icon={PieIcon} className="mb-0">
              Структура площадей
            </SectionTitle>
          </div>
          ...
          <div className="flex flex-col md:flex-row items-center gap-8 flex-1">
            <div className="h-[300px] w-full md:w-1/2 relative">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none pb-8">
                ...
                <span className="text-2xl font-black text-slate-800">
                  {stats.totalAreaProj.toLocaleString(undefined, { maximumFractionDigits: 0 })}
                </span>
            ...
            <div className="w-full md:w-1/2 space-y-3">
              {chartData.map((item, idx) => (
                <div
                  key={idx}
                  className="group flex items-center justify-between p-3 rounded-xl border border-slate-100 hover:bg-slate-50 transition-colors"
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-sm font-bold text-slate-700">
                        {item.value.toLocaleString(undefined, { maximumFractionDigits: 1 })} м²
                      </div>
                    </div>
                  </div>
                  <div className="text-xs font-mono font-bold text-slate-400">
                    {stats.totalAreaProj > 0
                      ? ((item.value / stats.totalAreaProj) * 100).toFixed(1)
                      : 0}
                    %
                  </div>
                </div>
        ...
        <div className="space-y-6 flex flex-col">
          <Card
            className={`p-6 shadow-sm border-l-4 ${isDiffCritical ? 'border-l-red-500' : 'border-l-emerald-500'}`}
          >
            ...
            <div className="mt-2 pt-3 border-t border-slate-100 flex justify-between items-center">
              ...
              <span
                className={`text-sm font-bold ${diff > 0 ? 'text-red-600' : 'text-emerald-600'}`}
              >
                {diff > 0 ? '+' : ''}
                {diff.toLocaleString(undefined, { maximumFractionDigits: 1 })} м² (
