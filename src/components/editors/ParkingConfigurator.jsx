...
import { Save, Car, CheckCircle2, Loader2, ArrowDown, ArrowUp, ArrowDownToLine, Footprints, MapPin, Building2, X } from 'lucide-react';
...
import { Card, Button, Input, useReadOnly, Label, DebouncedInput } from '../ui/UIKit'; 
...
const ParkingRow = ({ row, isReadOnly, counts, basements, onToggle, onCountChange }) => {
    ...
    return (
        <tr className={`transition-colors ${isEnabled ? 'bg-white' : 'bg-slate-50/50'}`}>
            <td className="p-4">
                <span className="text-xs font-bold text-slate-600 bg-slate-100 px-2 py-1 rounded border border-slate-200">{row.houseNumber}</span>
            </td>
            <td className="p-4 text-sm font-bold text-slate-700">{row.buildingLabel}</td>
            <td className="p-4"><span className="text-xs font-medium text-slate-500">{row.blockLabel}</span></td>
            <td className="p-4"><span className={`text-[10px] font-bold uppercase px-2 py-1 rounded border ${row.type === 'Подземный' ? 'bg-slate-800 text-white border-slate-800' : row.type === 'Подвал' ? 'bg-slate-100 text-slate-500 border-slate-200' : 'bg-blue-100 text-blue-700 border-blue-200'}`}>{row.type}</span></td>
            <td className="p-4"><div className="flex flex-col"><span className={`font-bold text-sm ${isEnabled ? 'text-slate-800' : 'text-slate-400'}`}>{row.label}</span></div></td>
            
            <td className="p-4 text-center">
                <label className={`flex items-center justify-center group relative ${row.isMandatory || isReadOnly ? 'cursor-not-allowed opacity-80' : 'cursor-pointer'}`}>
                    <input type="checkbox" className="peer sr-only" checked={isEnabled} disabled={row.isMandatory || isReadOnly} onChange={handleToggle}/>
                    {row.isMandatory ? (
...
export default function ParkingConfigurator({ buildingId }) {
    ...
    const { basements, counts, toggleLevel, setPlacesCount, isMutating } = useDirectParking(projectId);
                ...
                if (isParkingBuilding) {
                    ...
                    if (isUnderground) {
                         for(let i=1; i<=floorsCount; i++) {
                             rows.push({ ...commonData, id: `level_minus_${i}`, label: `Уровень -${i}`, type: 'Подземный', isMandatory: true, isBasement: false });
                         }
                    } else {
                        for(let i=1; i<=floorsCount; i++) {
                            rows.push({ ...commonData, id: `floor_${i}`, label: `Этаж ${i}`, type: 'Наземный', isMandatory: true, isBasement: false });
                        }
                        const blockBasements = basements.filter(base => base.blockId === block.id);
                        blockBasements.forEach(base => {
                            for (let d = 1; d <= base.depth; d++) {
                                rows.push({ ...commonData, id: `base_${base.id}_L${d}`, label: `Подвал -${d}`, type: 'Подвал', basementId: base.id, depthLevel: d, isMandatory: true, isBasement: true });
                            }
                        });
                    }
                } else {
                    ...
                    blockBasements.forEach(base => {
                         for (let d = 1; d <= base.depth; d++) {
                             rows.push({ ...commonData, id: `base_${base.id}_L${d}`, label: `Подвал -${d}`, type: 'Подвал', basementId: base.id, depthLevel: d, isMandatory: false, isBasement: true });
                         }
                    });
                }
            });
    ...
    if (loadingBuildings) return <div className="p-12 flex justify-center"><Loader2 className="animate-spin text-blue-600"/></div>;
    ...
    return (
        <div className="max-w-7xl mx-auto pb-20 animate-in fade-in duration-500 px-6">
            <div className="flex items-center justify-between border-b border-slate-200 pb-6 mb-6">
                ...
                {isMutating && <div className="text-blue-600 text-xs font-bold animate-pulse flex items-center gap-2"><Loader2 size={14} className="animate-spin"/> Сохранение...</div>}
            </div>
            ...
            {allRows.length > 0 ? (
                ...
                <div className="p-12 text-center text-slate-400 border-2 border-dashed border-slate-200 rounded-2xl bg-slate-50/50">
                    ...
                    <p className="text-xs mt-2 max-w-sm mx-auto">Убедитесь, что в проекте есть паркинги или жилые дома с подвалами.</p>
                </div>
            )}
        </div>
    );
