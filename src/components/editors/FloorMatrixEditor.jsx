...
export default function FloorMatrixEditor({ buildingId, onBack }) {
  ...
  const renderTypeBadge = type => {
    ...
    const labels = {
      ...
      attic: 'Мансарда',
      office: 'Нежилой',
      parking_floor: 'Паркинг',
    };
    return (
      <span
        className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded border ${styles[type] || styles.residential}`}
      >
        {labels[type] || type}
      </span>
    );
      ...
      <div className="space-y-6 pb-20 w-full px-6 animate-in fade-in">
        ...
        <div className="flex items-start gap-3 rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-amber-900">
          ...
          <p className="text-sm leading-6">
            В этом жилом блоке этажи стилобата скрыты на шаге «Внешняя инвентаризация». Заполняйте
            данные по ним в связанном нежилом блоке.
          </p>
        </div>
      </div>
    );
        ...
        <div className="sticky top-4 z-50 mx-auto max-w-2xl animate-in slide-in-from-top-4 fade-in duration-300">
          <div className="bg-slate-900/95 backdrop-blur text-white p-2.5 rounded-2xl shadow-2xl flex items-center gap-3 border border-slate-700/50 ring-1 ring-white/10">
            ...
            <div className="flex gap-1.5">
              <button
                onClick={() => applyBulk('height')}
                className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 rounded-lg text-xs font-bold transition-colors border border-white/10"
              >
                Высота
              </button>
              <button
                onClick={() => applyBulk('areaProj')}
                className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 rounded-lg text-xs font-bold transition-colors border border-white/10"
              >
                Проект
              </button>
              <button
                onClick={() => applyBulk('areaFact')}
                className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 rounded-lg text-xs font-bold transition-colors border border-white/10"
              >
                Факт
              </button>
            ...
            <button
              ...
              className="ml-auto p-2 hover:bg-white/10 rounded-lg text-slate-400 hover:text-white transition-colors"
            >
              <X size={16} />
            </button>
          </div>
              ...
              <tr className="bg-slate-100 border-b-2 border-slate-300">
                <th className="p-0 w-12 text-center border-r border-slate-300 sticky left-0 z-40 bg-slate-100">
                  <div className="h-full w-full flex items-center justify-center">
                    <button
                      disabled={isReadOnly}
                      onClick={toggleAll}
                      className={`text-slate-500 hover:text-blue-600 transition-colors p-2 ${isReadOnly ? 'cursor-not-allowed opacity-50' : ''}`}
                    >
                      {selectedRows.size === visibleFloors.length && visibleFloors.length > 0 ? (
                ...
                <th className="px-4 py-3 w-48 text-center border-r border-slate-300 bg-slate-100 text-xs font-bold text-slate-700 uppercase tracking-wider">
                  <div className="flex items-center justify-center gap-1.5">
                    <Ruler size={14} className="text-slate-500" /> Высота (м)
                  </div>
                </th>
                <th className="px-4 py-3 w-48 text-center border-r border-slate-300 bg-slate-100 text-xs font-bold text-slate-700 uppercase tracking-wider">
                  <div className="flex items-center justify-center gap-1.5">
                    <FileText size={14} className="text-slate-500" /> Площадь по проекту (м²)
                  </div>
                </th>
                <th className="px-4 py-3 w-48 text-center bg-slate-100 text-xs font-bold text-slate-700 uppercase tracking-wider">
                  <div className="flex items-center justify-center gap-1.5">
                    <Maximize2 size={14} className="text-slate-500" /> Площадь по обмерам (м²)
                  </div>
                </th>
              </tr>
            </thead>
              ...
              {visibleFloors.map((f, idx) => {
                ...
                return (
                  <tr key={f.id} className={`${rowBg} hover:bg-blue-50/30 transition-colors group`}>
                    <td className="p-0 text-center border-r border-slate-200 sticky left-0 z-20 bg-inherit">
                      <div className="h-full w-full flex items-center justify-center backdrop-blur-sm">
                        <button
                          ...
                          className={`text-slate-300 hover:text-blue-600 transition-colors p-2 ${isReadOnly ? 'cursor-not-allowed opacity-50' : ''}`}
                        >
                          {isSelected ? (
                            <CheckSquare size={18} className="text-blue-600" />
                          ) : (
                    ...
                    <td
                      className={`px-4 py-3 border-r border-slate-200 sticky left-12 z-20 bg-inherit shadow-[4px_0_8px_-4px_rgba(0,0,0,0.05)] relative group/label ${openMenuId === f.id ? 'z-50' : ''}`}
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          {f.isTechnical && (
                            <ArrowUpFromLine size={14} className="text-amber-500" />
                          )}
                        ...
                        {!isReadOnly && (
                          <div
                            className={`opacity-0 group-hover/label:opacity-100 transition-opacity ${openMenuId === f.id ? 'opacity-100' : ''}`}
                          >
                            <button
                              onClick={e => {
                                e.stopPropagation();
                      ...
                      {openMenuId === f.id && !isReadOnly && (
                        <div
                          ref={menuRef}
                          className="absolute left-[85%] top-8 w-52 bg-white rounded-xl shadow-xl border border-slate-200 z-[100] p-1.5 flex flex-col animate-in fade-in zoom-in-95 duration-200 origin-top-left"
                        >
                          <button
                            onClick={() => copyRowFromPrev(idx)}
                            disabled={idx === 0}
                            className={`flex items-center gap-2 px-3 py-2.5 text-xs font-bold rounded-lg text-left transition-colors ${idx === 0 ? 'text-slate-300 cursor-not-allowed' : 'text-slate-700 hover:bg-slate-50 hover:text-blue-600'}`}
                          >
                            <ArrowUp
                              size={14}
                              className={idx === 0 ? 'text-slate-300' : 'text-blue-500'}
                            />{' '}
                            Скопировать с пред.
                          </button>
                          <div className="h-px bg-slate-100 my-1" />
                          <button
                            onClick={() => fillRowsBelow(idx)}
                            disabled={idx === visibleFloors.length - 1}
                            className={`flex items-center gap-2 px-3 py-2.5 text-xs font-bold rounded-lg text-left transition-colors ${idx === visibleFloors.length - 1 ? 'text-slate-300 cursor-not-allowed' : 'text-slate-700 hover:bg-slate-50 hover:text-blue-600'}`}
                          >
                            <ChevronsDown
                              size={14}
                              className={
                    ...
                    {[
                      ...
                      let borderClass =
                        'border-transparent focus:border-blue-500 focus:ring-4 focus:ring-blue-100';
                      ...
                      return (
                        <td
                          key={field.id}
                          className="p-2 border-r border-slate-200 relative group/input"
                        >
                          <div className="relative h-10">
                            ...
                            {!isReadOnly && (
                              <div className="absolute inset-y-0 right-1 flex flex-col justify-center gap-0.5 opacity-0 group-hover/input:opacity-100 transition-opacity z-10">
                                {idx > 0 && (
                                  <button
                                    onClick={() => copyFieldFromPrev(idx, field.id)}
                                    className="p-0.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded bg-white/80 shadow-sm border border-slate-200"
                                    title="Копировать верх"
                                  >
                                    <ArrowUp size={10} />
                                  </button>
                                )}
                                {idx < visibleFloors.length - 1 && (
                                  <button
                                    onClick={() => fillFieldBelow(idx, field.id)}
                                    className="p-0.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded bg-white/80 shadow-sm border border-slate-200"
                                    title="Заполнить низ"
                                  >
