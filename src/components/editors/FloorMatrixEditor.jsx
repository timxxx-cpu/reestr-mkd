...
export default function FloorMatrixEditor({ buildingId, onBack }) {
    ...
    const handleInput = useCallback((floorId, field, value) => { 
        // ИСПРАВЛЕНИЕ: Запрещаем ввод отрицательных значений и знака минус
    ...
    const hasCriticalErrors = useMemo(() => {
        for (const f of floorList) {
            ...
            // ИСПРАВЛЕНИЕ: Явное указание типа для устранения ошибки TS2339
            ...
            if (heightError || areaProjError || areaFactError) return true;
        }
        return false;
    }, [floorList, floorData, currentBlock.fullId]);

    // --- НОВОЕ: Навигация стрелками ---
                ...
                <div className="sticky top-4 z-50 mb-4 mx-auto max-w-2xl animate-in slide-in-from-top-4 fade-in duration-300">
                    <div className="bg-slate-900 text-white p-2 rounded-xl shadow-2xl flex items-center gap-3 border border-slate-700">
                        ...
                        <Input 
                            value={bulkValue} 
                            onChange={(e) => {
                                // ИСПРАВЛЕНИЕ: Блокировка отрицательных значений в массовом вводе
                            ...
                            // ИСПРАВЛЕНИЕ: Добавлен focus:bg-slate-800 для предотвращения белого фона на белом тексте
                                            ...
                                            [
                                                ...
                                                return (
                                                    <td key={field.id} className="p-2 border-r border-slate-100 relative h-16 group/input">
                                                        <DebouncedInput 
                                                            // Пробрасываем ref для навигации
                                                            // @ts-ignore
                                                            ...
                                                            type="number" 
                                                            // ИСПРАВЛЕНИЕ: Добавлен min="0" для браузерной валидации
                                                            min="0"
                                                            step="0.01" 
