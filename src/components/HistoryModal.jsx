            ...
            >
                ...
                <div className="flex-1 overflow-y-auto p-6 bg-slate-50 space-y-4">
                    ...
                    {history?.map((item, idx) => {
                        const statusConfig = APP_STATUS_LABELS[item.action] || { label: item.action, color: 'bg-slate-100 text-slate-600' };
                            ...
                            <div key={idx} className="relative pl-6 pb-6 last:pb-0 border-l border-slate-200 last:border-0">
                                ...
                                <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm -mt-1.5">
                                    ...
                                    <div className="flex items-center gap-2 mb-3">
                                        {prevStatusConfig && (
                                            <>
                                                <span className={`text-[10px] px-2 py-0.5 rounded font-bold uppercase opacity-60 ${prevStatusConfig.color.replace('border-','border-transparent ')}`}>
                                                    {prevStatusConfig.label}
                                                </span>
                                                <ArrowRight size={12} className="text-slate-300"/>
                                            </>
                                        )}
                                        <span className={`text-[10px] px-2 py-0.5 rounded font-bold uppercase border ${statusConfig.color}`}>
                                            {statusConfig.label}
                                        </span>
                                    </div>
