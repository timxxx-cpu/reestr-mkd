...
import { CheckCircle2, Clock, XCircle, FileText, CheckSquare, Square, Check, Lock, ArrowRight, Flag, AlertTriangle, Loader2, History } from 'lucide-react';
...
const SavingOverlay = () => (
    <div className="fixed inset-0 z-[9999] bg-slate-900/50 backdrop-blur-sm flex items-center justify-center animate-in fade-in duration-200">
        <div className="bg-white px-8 py-6 rounded-2xl shadow-2xl flex flex-col items-center gap-4 border border-slate-100">
            <Loader2 className="animate-spin text-blue-600" size={40} />
            <div>
                <h3 className="text-lg font-bold text-slate-800 text-center">Сохранение данных</h3>
                <p className="text-slate-500 text-sm text-center">Пожалуйста, подождите...</p>
            </div>
        </div>
    </div>
);
...
export default function WorkflowBar({ user, currentStep }) {
  const { applicationInfo, updateStatus, setStepVerified, setStepCompleted, saveProjectImmediate } = useProject(); 
  ...
  const { status, currentStage, verifiedSteps = [], completedSteps = [], rejectionReason, history = [] } = applicationInfo;
  ...
  const handleTechToggle = async () => {
      ...
      try {
          ...
          console.error(error);
          toast.error("Ошибка при сохранении шага");
      } finally {
  ...
  return (
    <>
        {/* Окно сохранения */}
        {isSavingStep && <SavingOverlay />}
        
        {/* Модалка истории */}
        {isHistoryOpen && <HistoryModal history={history} onClose={() => setIsHistoryOpen(false)} />}

        {/* 1. ТЕХНИК */}
        {user.role === ROLES.TECHNICIAN && (
            <>
                {(status === APP_STATUS.NEW || status === APP_STATUS.DRAFT || status === APP_STATUS.REJECTED) ? (
                    <div className="flex flex-col sticky top-0 z-30 shadow-sm animate-in slide-in-from-top-2">
                        ...
                        <div className="bg-white border-b border-slate-200 px-8 py-3 flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                ...
                                {/* Кнопка Истории для Техника */}
                                <button onClick={() => setIsHistoryOpen(true)} className="ml-2 p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="История изменений">
                                    <History size={18} />
                                </button>
                            </div>
                            <div className="flex items-center gap-3">
                                {isStepInCurrentStage && (
                                    <div className="flex items-center animate-in fade-in">
                                        <button onClick={handleTechToggle} disabled={isSavingStep} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-bold border shadow-sm transition-all h-9 ${isCompletedByTech ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-white text-slate-600 border-slate-300 hover:border-blue-400'}`}>
                    ...
                    <div className="bg-amber-50 border-b border-amber-100 px-8 py-3 flex items-center justify-between sticky top-0 z-30">
                        <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-full bg-amber-100 text-amber-600 flex items-center justify-center animate-pulse"><Clock size={16} /></div>
                            <div><div className="text-xs font-bold text-amber-600/70 uppercase">Режим ожидания</div><div className="text-sm font-bold text-amber-700">На проверке у бригадира</div></div>
                            {/* Кнопка Истории (ReadOnly режим) */}
                            <button onClick={() => setIsHistoryOpen(true)} className="ml-2 p-1.5 text-amber-400 hover:text-amber-700 hover:bg-amber-100 rounded-lg transition-colors">
                                <History size={18} />
                            </button>
                        </div>
                        <div className="text-xs font-bold text-amber-600 bg-white/50 px-3 py-1.5 rounded-lg">Только чтение</div>
                    </div>
                ) : null}
            </>
        )}
        ...
        {/* 2. БРИГАДИР / АДМИН */}
        {(user.role === ROLES.CONTROLLER || user.role === ROLES.ADMIN) && (
            <>
                {status === APP_STATUS.REVIEW && (
                    <div className="bg-indigo-50 border-b border-indigo-100 px-8 py-3 flex items-center justify-between sticky top-0 z-30 animate-in slide-in-from-top-2">
                        <div className="flex items-center gap-3">
                            ...
                            {/* Кнопка Истории для Бригадира */}
                            <button onClick={() => setIsHistoryOpen(true)} className="ml-2 p-1.5 text-indigo-400 hover:text-indigo-700 hover:bg-indigo-100 rounded-lg transition-colors">
                                <History size={18} />
                            </button>
                        </div>
                        <div className="flex items-center gap-3">
