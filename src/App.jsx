...
function ProjectEditorRoute({ user }) {
  ...
  const renderStepContent = () => {
    if (editingBuildingId) {
      ...
      if (stepId === 'mop')
        return (
          <MopEditor buildingId={editingBuildingId} onBack={() => setEditingBuildingId(null)} />
        );
    ...
    switch (stepId) {
      ...
      case 'integration_buildings':
        return <IntegrationBuildings />;
      case 'integration_units':
        return <IntegrationUnits />;
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <ApplicationsDashboard
        ...
        projects={projects}
        dbScope={DB_SCOPE}
        onSelectProject={(id, mode) =>
          navigate(`/project/${id}${mode === 'view' ? '?mode=view' : ''}`)
        }
