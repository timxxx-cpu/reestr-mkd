...
export const ParticipantSchema = z.object({
  ...
  inn: z
    .string()
    .regex(/^\d{9,12}$/, 'ИНН должен содержать от 9 до 12 цифр')
    .optional()
    .or(z.literal('')),
  loading: z.boolean().optional(),
});

export const ComplexInfoSchema = z
  .object({
    name: z.string().min(3, 'Название должно быть не короче 3 символов'),
    status: z
      .enum(['Проектный', 'Строящийся', 'Готовый к вводу', 'Введенный', 'Архив'])
      .default('Проектный'),
...
export const BuildingConfigSchema = z
  .object({
    ...
    seismicity: z.coerce
      .number()
      .int()
      .min(3, 'Не менее 3 баллов')
      .max(10, 'Макс. 10 баллов')
      .optional(),
  ...
  .refine(
    data => {
      ...
      if (
        ...
        data.floorsFrom !== null &&
        data.floorsTo !== null &&
        data.floorsFrom > data.floorsTo
      )
        return false;
      return true;
    },
