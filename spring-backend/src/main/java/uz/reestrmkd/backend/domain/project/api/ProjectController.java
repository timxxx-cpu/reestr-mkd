...
public class ProjectController {
    ...
    public MapResponseDto createFromApplication(
            ...
            @CurrentUser ActorPrincipal actor // <-- Внедрили пользователя!
    ) {
      ...
      @SuppressWarnings("unchecked")
        ...
        String assignee = actor == null ? null : actor.userId(); // <-- Используем внедренного
        ...
        jdbcTemplate.update(
            ...
            externalId,
            applicant,
            java.sql.Timestamp.from(subInstant),
            assignee,
            "IN_PROGRESS",
            "DRAFT",
            0,
    ...
    public PagedItemsResponseDto getProjects(
        ...
        for (Map<String, Object> project : projects) {
            ...
            dto.put("availableActions", buildProjectAvailableActions(actor, dto)); // <-- Используем внедренного
            mapped.add(dto);
        }
