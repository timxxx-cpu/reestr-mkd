...
## 8) Реализованные улучшения (текущий Supabase стек)

По текущей итерации реализованы следующие пункты:

1. **Supabase-конфиг переведен в env-переменные**: `src/lib/supabase.js` читает `VITE_SUPABASE_URL` и `VITE_SUPABASE_ANON_KEY`; добавлен шаблон `.env.example`.  
2. **`ApiService` декомпозирован на доменные модули с фасадом**: добавлены `src/lib/api/project-api.js`, `src/lib/api/workflow-api.js`, `src/lib/api/registry-api.js`, при этом публичный импорт остается через `src/lib/api-service.js`.  
3. **Добавлены smoke-тесты критичных workflow-сценариев**: тестируются переходы complete/review/reject/rollback (`tests/workflow-smoke.test.mjs`, запуск через `npm run test:smoke`).  
4. **Усилена идемпотентность upsert-операций**: введен единый словарь `UPSERT_ON_CONFLICT` и использование согласованных ключей конфликтов в `ApiService`; отдельная таблица соответствий вынесена в `docs/upsert-on-conflict.md`.

## 9) Практический WorkFlow команды (как работать безопасно в DEV)
...
Если нужно, следующим шагом могу оформить этот backlog в виде чек-листа задач (P0/P1/P2) для команды.
