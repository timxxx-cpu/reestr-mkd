...
## 7) Важно по спорному месту #6 (граница этапа и старт интеграции на шаге 11)

### Текущее поведение

При завершении шага 11 одновременно верны условия:

- `isStageBoundary === true` (конец 3-го этапа),
- `isIntegrationStart === true` (следующий шаг — индекс 12).

Из-за приоритета переходов сначала срабатывает `REVIEW`, а не `INTEGRATION`.
Это поведение подтверждено smoke-тестом `complete step: boundary before integration keeps REVIEW status`.

### Предложения

#### Вариант A (рекомендуется как текущий baseline)
Оставить как есть: перед интеграцией всегда обязателен контрольный `REVIEW` на границе этапа 3.

**Плюсы:**
- контролируемое качество данных перед внешней интеграцией;
- не ломает текущие тесты и ожидания.

**Минусы:**
- UX может ожидать немедленного входа в `INTEGRATION` после шага 11.

#### Вариант B
Изменить приоритет и переводить сразу в `INTEGRATION` на переходе 11→12.

**Плюсы:**
- проще UX-переход для исполнителя.

**Минусы:**
- ослабляет контрольный шлюз перед интеграцией;
- потребует изменения `workflow-state-machine`, UI-подсказок и smoke-тестов.

#### Вариант C
Сохранить `REVIEW`, но добавить явный UX-баннер на шаге 12:
"Этап интеграции доступен после подтверждения контролером".

**Плюсы:**
- снимает неоднозначность без смены бизнес-логики.

**Минусы:**
- требует небольшого UI-доработки.

---

## 8) Ограничения

- Этот runbook только для DEV.
- Для Stage/Prod нужна миграционная стратегия без destructive `drop`.
