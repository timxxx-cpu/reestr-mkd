...
23. [Fullscreen CAD редактор блоков: frontend + варианты backend](./28-block-cad-editor-fullscreen-plan.md) ✨ **НОВЫЙ**
    - Полноэкранный CAD-редактор без картографической подложки
    - Автоповорот здания по узкой стороне и метровая сетка
    - Варианты backend: A (совместимый), B (доменный), C (event-sourcing)
