...
2. **Полнота данных МОП**:
   - Для каждого МОП должны быть заполнены `type`, `area` и `height`
   - Проверка: `!m.type OR !m.area OR parseFloat(m.area) <= 0 OR !m.height OR parseFloat(m.height) <= 0`
   - Ошибка: "У помещения №N не заполнен тип, площадь или высота"

**Таблицы БД**:
- `common_areas` (type, area, height, floor_id, entrance_id)
...
### 7.3.7 Шаг `registry_apartments` — Экспликация квартир (реестр)

**Проверки сохранения экспликации**:
- обязательны `тип комнаты`, `площадь`, `высота комнаты`;
- значения `площадь` и `высота` должны быть > 0;
- для не-дуплексных квартир `уровень комнаты` принудительно сохраняется как `1` и не редактируется в форме;
- для дуплексов доступен выбор уровня `1/2`;
- те же проверки применяются при массовом заполнении экспликации у выбранных квартир.


## 7.8 Валидация пристроек (`block_extensions`)

Для шагов `registry_res`/`registry_nonres` пристройки валидируются как отдельные step-сущности.

Проверяются базовые условия:

- `constructionKind` обязателен,
- `extensionType` обязателен,
- `floorsCount` — целое число `>= 1`,
- `verticalAnchorType` должен быть одним из `GROUND | BLOCK_FLOOR | ROOF`,
- при `BLOCK_FLOOR`/`ROOF` обязателен `anchorFloorKey`.

Статус заполнения рассчитывается отдельно для каждой пристройки:

- `FILLED` — данные есть и ошибок нет,
- `PARTIAL` — данные есть, но есть ошибки,
- `EMPTY` — данных нет.
