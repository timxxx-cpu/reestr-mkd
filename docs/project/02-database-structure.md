...
| `step_index` | INT | NOT NULL, UNIQUE(application_id, step_index) | (индекс) | **Номер шага (0-17)** | При создании | Индекс шага из STEPS_CONFIG |
...
**Значения `category`**:
- `residential` — Жилой дом (1+ жилых блоков, без нежилых блоков)
...
**Назначение**: Представляет структурные блоки внутри здания. Жилой дом может быть как одноблочным, так и многоблочным. Для категории `residential` допускается несколько **жилых** блоков без создания нежилых блоков. Для категории `residential_multiblock` также допустима многоблочная структура, включая нежилые блоки при необходимости.
