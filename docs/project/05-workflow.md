...
На вкладке «Входящие» реализована кнопка эмуляции **«Повторная подача»**.
Такая заявка формируется по существующему ЖК и содержит кадастровый номер `cadastre`.

### 5.6.2 Бизнес-правило принятия

Перед выполнением `createProjectFromApplication` выполняется проверка:

- есть ли по тому же ЖК активная заявка в `applications.status = 'IN_PROGRESS'`.

Проверка выполняется:

1. по `reapplicationForProjectId` (если есть),
2. иначе — по нормализованному кадастровому номеру ЖК (`formatComplexCadastre(cadastre)`).

### 5.6.3 Результат проверки

#### Сценарий A: активная заявка найдена

- Новая повторная заявка **не принимается**.
- Новый проект **не создается**.
- Возвращается отказ с текстом:
  - `Отказ в принятии: ... уже есть активное заявление в работе. Повторная подача отклонена.`
- Во входящих UI строка получает статус `DECLINED`, кнопка «Принять» блокируется.

#### Сценарий B: активной заявки нет

- Повторная заявка принимается в штатном режиме.
- Создается новый `projects` + связанная `applications`.

### 5.6.4 UI-поведение во «Входящих» для повторной подачи

Добавлены/обновлены элементы:

- кнопка **«Повторная подача»**;
- колонка **«Кадастровый номер ЖК»**;
- колонка **«Статус»** (`Новая`/`Отказано`);
- для повторной подачи показывается подпись `Повторно по ЖК: <название>`;
- у отклоненных входящих `Принять` недоступно.

### 5.6.5 Почему именно так

Это предотвращает параллельный запуск двух «живых» процессов по одному и тому же ЖК,
что снижает риск конфликтных изменений и ошибок синхронизации.

## 5.7 Назначение исполнителя, передача и блокировка конкурентной работы

### 5.7.1 Ограничение «взятия в работу» не своим исполнителем

Для роли `technician` действует правило:

- если `applications.assignee_name` заполнен и не совпадает с текущим пользователем,
  открыть заявку в режиме редактирования нельзя.

UI-поведение:

- кнопка «Взять в работу» недоступна по факту (выдаётся предупреждение);
- редактирование данных заблокировано.

### 5.7.2 Передача заявки между техниками начальником филиала

Роли `branch_manager` и `admin` могут выполнить передачу заявки:

1. выбрать заявку в рабочем столе;
2. нажать действие «Передать заявку другому технику»;
3. выбрать нового исполнителя из списка техников;
4. система обновляет `applications.assignee_name`.

Это позволяет «забрать» задачу у одного исполнителя и передать другому без смены статуса заявки.

### 5.7.3 Блокировка одновременного редактирования (work lock)

Реализована lock-модель через отдельную таблицу `application_locks` и аудит `application_lock_audit`.

Ключевые поля lock:

- `application_id` — уникальная ссылка на заявку;
- `owner_user_id` — владелец блокировки;
- `owner_role` — роль владельца;
- `acquired_at` — время захвата;
- `expires_at` — время истечения блокировки.

#### Алгоритм

1. При входе в проект в edit-режиме `technician/admin` вызывается атомарная SQL-функция `acquire_application_lock`.
2. Если lock активен и принадлежит другому пользователю — функция возвращает `LOCKED`, открытие в режиме редактирования запрещается.
3. Если заявка назначена другому технику — возвращается `ASSIGNEE_MISMATCH`.
4. Пока пользователь в проекте — вызывается `refresh_application_lock` (heartbeat).
5. При выходе вызывается `release_application_lock`.
6. Все события (ACQUIRE/REFRESH/RELEASE/DENY) пишутся в `application_lock_audit`.

### 5.7.4 Почему это важно

Механизм снижает риск параллельного редактирования одной и той же заявки двумя пользователями,
исключая конфликтные сохранения и «перетирание» данных.
