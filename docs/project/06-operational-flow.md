...
## 5.1) Инвентаризация подвалов (`basements`)

- `basements` + `basement_parking_levels` -> **Подвальные уровни и обслуживаемые блоки**.
- Подвальные блоки **не настраиваются** на шаге `registry_nonres`; их конфигурация вынесена в отдельный шаг `basements`.
